...
public class TempConverter {
	...
	public OrderDto orderEntityToDto(OrderEntity orderEntity) {
		...
		Optional<Timestamp> createTimeOpt = Optional.ofNullable(orderEntity.getCreateTime());
		if (createTimeOpt.isPresent()) {
			ZonedDateTime createTime = createTimeOpt.get().toLocalDateTime().atZone(zoneId);
	...
	public OrderItemDto cartItemToOrderItemDto(CartItemDto cartItemDto) {
		OrderItemDto returnValue = mapper.map(cartItemDto, OrderItemDto.class);

		Optional<Integer> pizzaSizeIdOptional = Optional.ofNullable(cartItemDto.getPizzaSizeId());
		if (pizzaSizeIdOptional.isPresent()) {
			Integer pizzaSizeId = pizzaSizeIdOptional.get();
			PizzaSizeEntity pizzaSize = pizzaSizeRepository.findById(pizzaSizeId).get();
			Optional<PizzaEntity> pizzaOptional = Optional.ofNullable(pizzaSize.getPizza());
			if (pizzaOptional.isPresent()) {
				returnValue.setPizza(pizzaOptional.get().getName());
				returnValue.setPizzaSize(pizzaSize.getName());
				returnValue.setPizzaPrice(pizzaSize.getPrice());
				returnValue.setPrice(pizzaSize.getPrice() * returnValue.getQuantity());
				Float price = Float.valueOf(decfor.format(returnValue.getPrice()));
				returnValue.setPrice(price);
			}
		}
	...
	public UserEntity userDtoToEntity(UserDto userDto) {
		...
		List<Integer> rolesIds = userDto.getRolesIds();
